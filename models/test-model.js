...
module.exports = (dbPoolInstance) => {
  ...
  let getUserExpense = (data, testCallback) => {
    ...
    let queryString = `
                      SELECT amount, user_id, to_char(date, 'YYYY-MM-dd') as date, category, comments, users.id, username, need_want
                      FROM expenses
                      INNER JOIN users
    ...
    //             let CategorizedExpenseResult = result.rows;
                ...
                testCallback(chosenPeriodString, listExpenseResult);
    //           }
    //         });
    //       }
    //     });
  ...
  return {
    testModelFunction : getUserExpense,
  };
};
